...
class LoggerWrapTest extends CatsEffectSuite {
  ...
  test("Logger should execute logging operations and capture them") {
    for {
      ref <- Ref.of[IO, List[Log[String]]](List.empty)
      kernel = testKernel[IO](ref)
      logger = Logger.wrap(kernel)
  ...
  test("Logger should support simple message logging") {
    for {
      ref <- Ref.of[IO, List[Log[String]]](List.empty)
      kernel = testKernel[IO](ref)
      logger = Logger.wrap(kernel)
  ...
  test("Logger should support error logging") {
    for {
      ref <- Ref.of[IO, List[Log[String]]](List.empty)
      kernel = testKernel[IO](ref)
      logger = Logger.wrap(kernel)
  ...
  test("Logger should support multiple log calls") {
    for {
      ref <- Ref.of[IO, List[Log[String]]](List.empty)
      kernel = testKernel[IO](ref)
      logger = Logger.wrap(kernel)
  ...
  test("Logger withModifiedString should modify messages correctly") {
    for {
      ref <- Ref.of[IO, List[Log[String]]](List.empty)
      kernel = testKernel[IO](ref)
      logger = Logger.wrap(kernel)
  ...
  test("Logger withModifiedString should preserve other fields") {
    for {
      ref <- Ref.of[IO, List[Log[String]]](List.empty)
      kernel = testKernel[IO](ref)
      logger = Logger.wrap(kernel)
